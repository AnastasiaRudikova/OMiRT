...
int read_Adc(int NoSens)
{
    ADCON0bits.CHS=NoSens; // âûáîð àíàëîãîâîãî êàíàëà
...
// ïîèñê óñðåäíåííîãî çíà÷åíèÿ äàò÷èêà ïîä íîìåðîì NoSens
int trueValue(int NoSens){
    int cnt = 1000;   //êîë-âî çíà÷åíèé äëÿ óñðåäíåíèÿ
    long sumValue = 0;
    
    for(int i = 0; i < cnt ; i++){
        sumValue += read_Adc(NoSens);
    }
     
    return sumValue/cnt;
}

// îïðåäåëåíèå íîìåðà ñ÷èòûâàåìîãî äàò÷èêà 
// ïî ïîêàçàíèþ ïîòåíöèîìåòðà 
int getStage(){
    int value = trueValue(7);
    
    if(value < 147 ) return 0;
    if(value < 293 ) return 1;
    if(value < 440 ) return 2;
    if(value < 587 ) return 3;
    if(value < 733 ) return 4;
    if(value < 881 ) return 5;
    return 6;
}
...
int main(int argc, char** argv)
{
    ...
    Adc_init();
    lcd_init();
    lcd_puts(0x0,"sencor No");
    lcd_puts(0xC0,"Value=");
    while(1)
    {
        NoSens = getStage();
        inttolcd(0x8A,NoSens); // âûâîä íîìåðà äàò÷èêà
        value = trueValue(NoSens);
        inttolcd(0xC7,value); // âûâîä çíà÷åíèÿ äàò÷èêà
        
        if(value<1000){
         lcd_puts(0xCA," ");   // âñòàâêà ïóñòîãî ñèìâîëà ïîñëå çíà÷åíèÿ
        }
    }
}
